...
export async function GET(request: Request) {
  try {
    ...
    console.log('Power User Conversion API: Starting with filters:', { timeFilter, excludeTest });
    ...
    // Get user engagement data for current period
    const getCurrentPeriodData = async () => {
      ...
      if (!messageData) return { engagedUsers: 0, powerUsers: 0, conversionRate: 0 };
      ...
      const powerUsers = userCounts.filter(entry => entry[1] >= 10).length;
      
      const conversionRate = engagedUsers > 0 ? (powerUsers / engagedUsers) * 100 : 0;
    ...
    const getPreviousPeriodData = async () => {
      if (!start) return { engagedUsers: 0, powerUsers: 0, conversionRate: 0 };
    ...
    if (previousPeriod.conversionRate > 0) {
      percentChange = Math.round(((currentPeriod.conversionRate - previousPeriod.conversionRate) / previousPeriod.conversionRate) * 100);
      changeDirection = percentChange > 0 ? 'up' : percentChange < 0 ? 'down' : 'neutral';
    } else if (currentPeriod.conversionRate > 0) {
      percentChange = 100;
      changeDirection = 'up';
    }
    ...
    const result = {
      ...
      previousConversionRate: previousPeriod.conversionRate,
      percentChange: Math.abs(percentChange),
      changeDirection,
      timeFilter,
      excludeTest,
      currentEngagedUsers: currentPeriod.engagedUsers,
      currentPowerUsers: currentPeriod.powerUsers,
      previousEngagedUsers: previousPeriod.engagedUsers,
      previousPowerUsers: previousPeriod.powerUsers
    };

    console.log('Power User Conversion API: Result:', result);

    return NextResponse.json(result);

  } catch (error) {
    console.error('Power User Conversion API: Error:', error);
    return NextResponse.json({ 
      error: 'Failed to fetch power user conversion data',
      conversionRate: 0,
